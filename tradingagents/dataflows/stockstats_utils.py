...
def _single_case_ohlcv(data: pd.DataFrame) -> pd.DataFrame:
    """Drop capitalized OHLCV columns that already have a lowercase twin.

    stockstats.wrap() lowercases every column name, so a frame carrying both
    ``Close`` and ``close`` ends up with two identical columns and every
    indicator fails with "Expected a single column, got 2".
    """
    duplicated = [
        column
        for column in _OHLCV_CASE_MAP
        if column in data.columns and column.lower() in data.columns
    ]
    return data.drop(columns=duplicated) if duplicated else data


def _add_lowercase_aliases(data: pd.DataFrame) -> pd.DataFrame:
    """Lowercase aliases for the manual OBV/ATR paths, which read them off
    the raw frame rather than the stockstats wrapper."""
    for cap, low in _OHLCV_CASE_MAP.items():
...
class StockstatsUtils:
    ...
    def get_stock_stats(
        ...
        if not online:
            ...
            end_date = min(pd.Timestamp.today().normalize(), curr_date_dt.normalize())
            ...
            try:
                if os.path.exists(data_file):
                    ...
                    data = _add_lowercase_aliases(data)
                else:
                ...
                return f"N/A: Error fetching data for {symbol}: {str(e)}"

        # Shared by both branches. This block used to sit inside the online
        # branch, so an offline call fell off the end of the function and
        # returned None for every indicator.
        try:
            data = _add_lowercase_aliases(data)
            ...
            df = wrap(_single_case_ohlcv(data))
