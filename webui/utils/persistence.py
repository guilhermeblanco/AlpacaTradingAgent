...
30s, for every open browser tab. Building a runtime per tick creates and
disposes a SQLAlchemy engine each time, so every refresh pays a fresh TCP
connect and authentication handshake and connection pooling never applies.
...
def _runtime_key(config: dict) -> tuple:
...
def get_persistence_runtime():
    ...
    with _LOCK:
        ...
        _close_locked()
        _CACHED_RUNTIME = build_persistence_runtime(config)
        _CACHED_KEY = key
        return _CACHED_RUNTIME
